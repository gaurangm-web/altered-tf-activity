...
<br />2) <sup>b</sup>The obtained match (log p-value) is compared with a null distribution which is generated by repeated randomization of the original DEG\
 set. A large deviation from this distribution (large -ve z-score) indicates a concordance between the identified TF combination\
...
<br />5) The uploaded data is not stored by the app.",unsafe_allow_html=True)
    st.markdown("***")
    st.markdown("Created by Gaurang Mahajan. Any [feedback](https://gaurangm-web.github.io) on use of this tool or suggested improvements will be appreciated.",unsafe_allow_html=True)
